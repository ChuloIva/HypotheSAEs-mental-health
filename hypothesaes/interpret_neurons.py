...
class NeuronInterpreter:
    def __init__(
        ...
        annotator_model: str = "gpt4.1-nano",
        ...
        for neuron_idx, neuron_interps in interpretations.items():
            ...
            for interp in neuron_interps:
                try:
                    ...
                    print(f"Warning: Missing annotation for interpretation '{interp}' and text (truncated): {str(e)[:100]}...")
                    print(f"Skipping metrics computation for neuron {neuron_idx}, interpretation: {interp}")
                    all_metrics[neuron_idx][interp] = {
                        'recall': 0.0,
                        'precision': 0.0,
                        'f1': 0.0,
                        'correlation': 0.0
